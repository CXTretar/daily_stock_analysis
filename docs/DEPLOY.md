...
默认配置：**每天，北京时间 08:30** 主触发，并在 09:17、10:47 增加备份触发。备份触发用于降低 GitHub Actions `schedule` 延迟或丢弃导致当天漏跑的风险；workflow 会按北京时间日期写入成功标记，同一天后续定时触发会自动跳过。
