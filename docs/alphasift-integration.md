...
- 依赖来源：`requirements.txt` 固定到已验证的 AlphaSift 适配层 commit：`git+https://github.com/CXTretar/alphasift.git@271385db8f1f6cf8c45da16f0db8b6e15938d090#egg=alphasift`（对应提交 `https://github.com/CXTretar/alphasift/commit/271385db8f1f6cf8c45da16f0db8b6e15938d090`）。该来源覆盖 `alphasift.dsa_adapter` 契约、`screen/list_strategies/get_status` 调用，以及 Tencent 日 K、Sina snapshot、source health、stale daily fallback、候选级 quote context 与 `volume_breakout` 缺少实时量比时的日线放量兜底。
...
- 外部契约依据：本次 AlphaSift 运行契约（含 `schema_version=2` 的热点缓存与题材详情字段）对应 GitHub 提交 `https://github.com/CXTretar/alphasift/commit/271385db8f1f6cf8c45da16f0db8b6e15938d090`。
...
AlphaSift 侧已在 `CXTretar/alphasift@271385db8f1f6cf8c45da16f0db8b6e15938d090` 提供 DSA provider context 支持、DSA adapter contract，并支持复用 DSA 的 `LLM_TIMEOUT_SEC`。
...
- 兼容语义与版本证据（可追溯）：
  - 运行依赖约束：`requirements.txt` 中将 LiteLLM 固定到 `litellm>=1.80.10,!=1.82.7,!=1.82.8,<2.0.0`，并通过 `git+https://github.com/CXTretar/alphasift.git@271385db8f1f6cf8c45da16f0db8b6e15938d090` 安装 AlphaSift 适配层。
  - 文档依据：
...
- 外部版本来源：本次集成依赖来源为 `https://github.com/CXTretar/alphasift/commit/271385db8f1f6cf8c45da16f0db8b6e15938d090`，需在复验时按该 commit pin 回放导入与接口契约。
